...
function Update() {
	...
	if(!isPaused){
	    ...
	    if(Input.GetButtonDown ("Cancel") && Application.loadedLevel >= 3 && Application.loadedLevel <= 6 ){ //Make sure game only pasuses during levels
			Debug.Log('pause!');
			pause();
	}
    }else{
...
function LoseHealth () {
	...
	if (playerHealth == 0) { //if player dies
		...
		player.transform.Find("PlayerMesh").animation.Play("Death"); //re-enable when this is fixed.
		characterDeath();
	}
}
...
function OnGUI() {
	...
	if (Application.loadedLevel >= 3 && Application.loadedLevel <= 9 ) { //if we're in an actual game level
		if (playerAlive) {
			if(isPaused){
			...
			GUI.Label(Rect(Screen.width - 20, Screen.height - 40,75,75), "<color=white>" + lives + "</color>", whiteText); //fragment count
			 ...
			 if(isPaused == true){
			 	if(GUI.Button(new Rect(Screen.width/2,Screen.height/2,85,20),"Save")){
			 		SavePlayerData();
			 	}
			 }
		}else {
...
function SavePlayerData(){
	...
	//PlayerPrefs.SetInt("saveSlot",currentSave);
	PlayerPrefs.Save();
}
...
function LoadPlayerData(){
	...
	playerSavedLevel = PlayerPrefs.GetInt("level");
	character = PlayerPrefs.GetFloat("character");
	playerHealth = PlayerPrefs.GetInt("playerhealth");
	batteries = PlayerPrefs.GetInt("batteries");
	fragments = PlayerPrefs.GetInt("fragments");
	Debug.Log(playerSavedLevel);
	Application.LoadLevel(playerSavedLevel);
}