...
function OnGUI() {
	...
	if (Application.loadedLevel >= 4 && Application.loadedLevel <= 9 ) { //if we're in an actual game level
		if (playerAlive) {
			if(isPaused){
...
function SavePlayerData(){
	...
	PlayerPrefs.Save();
	Debug.Log("Character: "+PlayerPrefs.GetFloat("character"));
	...
	PlayerPrefs.Save();
	Debug.Log(PlayerPrefs.GetInt("batteries"));
