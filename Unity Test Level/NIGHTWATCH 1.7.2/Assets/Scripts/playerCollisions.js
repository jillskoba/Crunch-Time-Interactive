...
function OnTriggerEnter(hit : Collider) {
	...
	if(hit.tag == "enemy" && gameManager.GetComponent(GameManager).playerAlive == true){
	...
	if(hit.tag == "killzone" && gameManager.GetComponent(GameManager).playerAlive == true){
		gameManager.GetComponent(GameManager).characterDeath();
	}
	
	//For projectile shooter && environment damage
	if(hit.tag == "hurtful" && gameManager.GetComponent(GameManager).playerAlive == true){
		if(gameManager.GetComponent(GameManager).overchargeActive == false) {
			Debug.Log('Hit By Something Harmful!');
			gameManager.GetComponent(GameManager).LoseHealth();
		}
	}
}